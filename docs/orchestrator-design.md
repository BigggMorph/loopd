...
> ## 🚀 Revision 19 (2026-05-26) — merged_observing: blocking 6h 타이머 → non-blocking CI-완료 게이트
>
> 사용자 지적: "PR 완수 후 6시간 회귀 관찰 때문에 이론상 하루 최대 4개 밖에 개발 못한다." → **사실**. 머지 후 이슈가 `merged_observing`이 되면 `current_issue`가 6h 동안 그 이슈에 고정되고 `("merged_observing", _)` 케이스가 `return`으로 끝나 새 이슈를 못 집음. Step 5는 `current_issue`가 set이면 무조건 그 이슈로 라우팅 → 단일 인스턴스는 6h 창을 **직렬** 통과 (repo당 ~4/일).
>
> 분석: 6h는 테스트 커버리지 도구가 아니라 자율 머지 신뢰 버퍼이며 근거 없는 임시값(본 문서 §16). 테스트가 구조적으로 못 잡는 유일한 잔여 위험은 **merge-skew**(rebase 없이 `--squash --auto` 머지 → 독립 통과한 두 PR이 main에서 합쳐져 깨짐)인데, 이건 "6h 대기"가 아니라 **머지 커밋의 main CI 1회 확인**으로 해결됨.
>
> | 항목 | 변경 |
> |---|---|
> | current_issue | 머지 성공 즉시 `None` → 다음 wake에 새 이슈 픽 (완전 병렬, cascade 가드 없음 — 사용자 결정) |
> | 관찰 위치 | Step 6A `("merged_observing", _)` 케이스 → **Step −5 백그라운드 sweep** (current_issue 무관, 매 wake 실행) |
> | 종료 기준 | 고정 6h 타이머 → **CI-완료 게이트**: main squash 커밋 CI 통과→`done_final` / 실패·revert·cross-ref→`regression_detected` |
> | 안전 캡 | CI 미보고/CI 없는 repo → `merged_at + ORCHESTRATOR_OBSERVE_CAP_MIN`(기본 60분, env) 후 무사고면 `done_final` |
> | 폴 간격 | `ORCHESTRATOR_OBSERVE_POLL_MIN`(기본 5분, env) — 잦은 wake에 API 안 두드림 |
> | 회귀 라우팅 | inline AskUser(current_issue 의존) → `push_pending_question(target="regression:{pr_url}")` + Step −3 flush → `issue.regression_decision` 플래그 → Step −5가 적용 (`pr_audit_pending` 선례) |
> | completed_count | 단일 증가 지점 (Step −5, `status==merged_observing` 가드) — 기존 Step −5/케이스 간 이중·누락 카운트 불일치 해소 |
> | drift 정리 | design.md `test_received`가 merge→`done`으로 가던 것을 SKILL.md(merge→`merged_observing`)에 맞춰 일치시킴 |
>
> 신규 순수 헬퍼 `playbook_helpers.classify_ci_completion(pr_view, check_runs, combined_status)` → `pending_merge|closed_unmerged|no_checks|running|failed|passed`. 스키마 버전 bump 없음(`watch_list` top-level 기존, 신규 키는 per-entry로 `.get()` 방어 read).
...
| `merged_observing` (Rev 19) | 머지 후 백그라운드 CI-완료 게이트 관찰 중 (current_issue 비고정) | main squash 커밋 CI 통과 → `done_final` / 실패·외부 revert·cross-ref → `regression_detected` / 안전 캡(기본 60분) 무사고 → `done_final`. **Step −5 sweep이 advance, current_issue 무관** |
| `regression_detected` (Rev 19) | 회귀 의심 — 결정 요청을 pending_questions 큐에 푸시 (current_issue 미고정) | Step −3가 `regression_decision` 기록 → Step −5가 적용: revert→revert PR 가이드+`reverted` / keep→`done_final` / manual→`needs_human` |
...
| `test_received` | verdict 처리 중 | (a) `pass` safe → 머지 → `merged_observing` (Rev 19, +current_issue=None)<br>(b) `pass` risky / `uncertain` → `merge_pending`<br>(c) `fail` & rework<2 → `ready_for_dev`<br>(d) `fail` & rework=2 → `needs_human` |
...
### Step −5 — Post-merge observation sweep (Rev 19, was Rev 13 Round F watch-list expiry)
```python
# merged_observing 이슈를 advance하는 유일한 장소. 매 wake 실행, current_issue 무관.
# (관찰은 완전 non-blocking — merge 경로가 current_issue를 즉시 비우므로 lead는 계속
#  새 이슈를 픽하고, 머지된 PR은 여기서 백그라운드로 관찰된다.) current_issue 미접근.
cap_min  = playbook_helpers.observe_cap_minutes()    # ORCHESTRATOR_OBSERVE_CAP_MIN, 기본 60
poll_min = playbook_helpers.observe_poll_minutes()   # ORCHESTRATOR_OBSERVE_POLL_MIN, 기본 5
owner, repo_name = state.repo.split("/", 1)
survivors = []
for w in state.watch_list:
    # issue_num은 int, state.issues 키는 str — 반드시 get_issue로 정규화 조회.
    issue = orchestrator_state.get_issue(state, w.get("issue_num"))
    # sweep는 관찰 중(merged_observing) + 회귀 결정 대기(regression_detected) 둘 다 처리.
    # 그 외 상태(이미 종료/고아)는 prune.
    if not issue or issue.status not in ("merged_observing", "regression_detected"):
        continue

    # 0. regression_detected: 사용자 결정이 들어왔으면 적용, 아니면 대기 (current_issue 비의존)
    if issue.status == "regression_detected":
        decision = issue.get("regression_decision")
        if decision == "revert":
            # 실제 git revert는 worktree 필요 — 가이드 emit + revert PR 생성
            bash(f"gh pr create --repo {state.repo} --title 'Revert: {issue.title}' "
                 f"--body 'Auto-revert by orchestrator: regression on {w['pr_url']}\\n\\n"
                 f"Evidence: {json.dumps(issue.get('regression_evidence', {}))}' "
                 f"--head 'revert-{w.get('pr_branch') or issue.pr_branch}' --base main")
            emit_to_user("revert PR 가이드 emit — 실제 git revert는 수동 권장.")
            transition(issue, "reverted"); continue              # entry drop
        if decision == "keep":
            transition(issue, "done_final"); state.completed_count += 1; continue  # 단일 증가
        if decision == "manual":
            issue.failure_reason = "regression suspected — user chose manual review"
            transition(issue, "needs_human"); continue
        survivors.append(w); continue                            # 결정 대기 — 엔트리 유지

    # 1. 폴 간격 가드 — 최근 폴이면 gh 호출 없이 유지
    last_chk = w.get("last_checked_at")
    if last_chk and now() - datetime.fromisoformat(last_chk) < timedelta(minutes=poll_min):
        survivors.append(w); continue

    # 2. 머지 상태 + squash SHA 해석. 머지됐을 때만 CI 조회 (gh api 2회 절약).
    pr_view = json.loads(bash(
        f"gh pr view {w['pr_url']} --repo {state.repo} --json state,mergeCommit,mergedAt,headRefName").stdout)
    pr_view["sha"] = w.get("merge_commit_sha") or (pr_view.get("mergeCommit") or {}).get("oid")
    # pr_branch는 issue에 신뢰성 있게 기록되지 않으므로 PR의 headRefName에서 채워 캐시.
    w["pr_branch"] = w.get("pr_branch") or pr_view.get("headRefName")
    check_runs = combined = None
    if (pr_view.get("state") or "").upper() == "MERGED" and pr_view["sha"]:
        sha = pr_view["sha"]; w["merge_commit_sha"] = sha
        # per_page=100: 30개 초과 체크가 있는 커밋도 한 번에 (running/failed 판정 누락 방지)
        check_runs = json.loads(bash(
            f"gh api 'repos/{owner}/{repo_name}/commits/{sha}/check-runs?per_page=100'").stdout)
        combined = json.loads(bash(
            f"gh api 'repos/{owner}/{repo_name}/commits/{sha}/status?per_page=100'").stdout)
    cls = playbook_helpers.classify_ci_completion(pr_view, check_runs, combined)
    w["last_checked_at"] = now().isoformat()

    # 3. 머지 전/취소는 부가 프로브 전에 단락 (불필요한 gh 호출 회피)
    if cls == "pending_merge":
        survivors.append(w); continue                 # auto-merge 미완료 — 대기
    if cls == "closed_unmerged":
        issue.regression_evidence = {"reason": "pr_closed_unmerged", "detected_at": now()}
        _route_regression(state, issue, w); survivors.append(w); continue

    # 4. 부가 회귀 신호 (머지 확인됨 — 기존 Rev 13 A2 프로브, main CI 게이트와 OR)
    reverted = bool(w.get("pr_branch")) and int(bash(f"gh pr list --repo {state.repo} --search "
        f"'in:title \"Revert\" head:{w['pr_branch']}' "
        f"--json number --jq 'length'").stdout.strip() or "0") > 0
    paths = w.get("touched_paths") or []
    # touched_paths 비면 contains("")가 모든 bug에 매칭되는 false-positive → xref 생략
    xref = ""
    if paths:
        xref = bash(f"gh issue list --repo {state.repo} --state open --label bug "
            f"--search 'created:>{w['merged_at']}' --json title,body "
            f"--jq '.[] | select(.body | contains(\"{paths[0]}\"))' | head -3").stdout.strip()

    # 5. 분기
    if cls == "failed" or reverted or xref:
        issue.regression_evidence = {"ci": cls, "reverted_externally": reverted,
            "new_bugs": xref[:500], "merge_commit_sha": pr_view["sha"], "detected_at": now()}
        _route_regression(state, issue, w); survivors.append(w); continue
    if cls == "passed":
        transition(issue, "done_final"); state.completed_count += 1; continue  # 단일 증가
    # cls in {"no_checks", "running"} → 안전 캡까지 대기
    if now() - datetime.fromisoformat(w["merged_at"]) >= timedelta(minutes=cap_min):
        transition(issue, "done_final"); state.completed_count += 1; continue   # 캡 도달, 무사고
    survivors.append(w)

state.watch_list = survivors
orchestrator_state.write(state)

# 회귀 라우팅 — current_issue 미접근 (pr_audit_pending 선례)
def _route_regression(state, issue, w):
    transition(issue, "regression_detected")
    push_pending_question(state, {
        "question": f"#{issue.number} PR {w['pr_url']} 머지 후 회귀 의심 "
                    f"({list(issue.regression_evidence.keys())}). 어떻게 할까?",
        "options": [{"label":"revert","description":"자동 revert PR 가이드 생성"},
                    {"label":"keep","description":"오탐. 머지 유지 → done_final"},
                    {"label":"manual","description":"사람 검토 필요 → needs_human"}],
        "target": f"regression:{w['pr_url']}",   # dedup — 같은 PR 중복 질문 방지
    })
```
**completed_count 단일 증가:** merge→`done_final` 경로(CI passed, 안전 캡, keep 결정)는 이 sweep에서만
카운트한다. 각 bump은 루프 상단의 상태 가드(`merged_observing`/`regression_detected`) 뒤에서만 실행되고,
`done_final`로 전이된 이슈의 엔트리는 survivors에 re-append되지 않으므로 sweep 재실행/중복 엔트리에도
재카운트되지 않는다. `done` 케이스는 비관찰 `done` 경로만 카운트(이중 카운트 없음).
...
# apply_answers_to_state — target 접두사로 라우팅:
#   stale_pr:<num>     → 매칭 이슈.audit_decision = answer       (pr_audit_pending이 읽음)
#   regression:<pr_url>→ 매칭 이슈.regression_decision = answer  (Rev 19; Step −5가 다음 sweep에 적용)
def apply_answers_to_state(state, questions, answers):
    for q, a in zip(questions, answers):
        target = q.get("target") or ""
        if target.startswith("stale_pr:"):
            iss = next((i for i in state.issues.values()
                        if i.get("pr_url") and target.endswith(str(i.get("number")))), None)
            if iss: iss["audit_decision"] = a
        elif target.startswith("regression:"):
            pr_url = target.split("regression:", 1)[1]
            iss = next((i for i in state.issues.values() if i.get("pr_url") == pr_url), None)
            if iss: iss["regression_decision"] = a   # "revert" | "keep" | "manual"
        # … 기타 target 처리

# dangerous-merge 확인만 큐 우회 즉시 호출. 회귀 확인은 Rev 19부터 큐 경유(관찰 non-blocking 유지).
```
...
while True:
  match (issue.status, wake_reason):
      ...
      if v.verdict == "pass":
        ...
        if not merged_ok:
          ...
          if merge_result.exit_code == 0:
            ...
            transition(issue, "needs_human")
        if merged_ok:
          # Rev 19 (drift fix): 과거엔 여기서 곧장 done이었으나, SKILL.md와 일치시켜
          # 백그라운드 관찰(merged_observing) 진입 + current_issue 즉시 해제.
          state.watch_list.append({
            "pr_url": issue.pr_url, "issue_num": issue.number,
            "pr_branch": issue.pr_branch, "touched_paths": issue.touched_paths or [],
            "merged_at": now().isoformat(), "merge_commit_sha": None, "last_checked_at": None,
            "expires_at": (now() + timedelta(minutes=playbook_helpers.observe_cap_minutes())).isoformat(),
          })
          transition(issue, "merged_observing")
          state.current_issue = None           # Rev 19: 다음 wake에 새 이슈 픽 (완전 병렬)
        continue
      if v.verdict == "fail":
    ...
    case ("merge_pending", _):
      ...
      if wake_reason[0] == "user_input":
        ...
        if ans == "yes":
          ...
          if not was_risky:
            state.auto_merge_consecutive_safe += 1
          # Rev 19: 머지 후 백그라운드 관찰 진입 (non-blocking)
          state.watch_list.append({
            "pr_url": issue.pr_url,
            "issue_num": issue.number,
            "pr_branch": issue.pr_branch,
            "touched_paths": issue.touched_paths or [],
            "merged_at": now().isoformat(),
            "merge_commit_sha": None,          # Rev 19: MERGED 확인 시 1회 채움
            "last_checked_at": None,           # Rev 19: 폴 간격 가드
            "expires_at": (now() + timedelta(minutes=playbook_helpers.observe_cap_minutes())).isoformat(),  # Rev 19: 의미 = 안전 캡(기본 60분), 기다리는 시간 아님
          })
          transition(issue, "merged_observing")
          state.current_issue = None           # Rev 19: 즉시 비워 다음 wake에 새 이슈 픽
        else:
    ...
    case ("merged_observing", _):
      # Rev 19: 관찰은 Step −5 백그라운드 sweep이 전담(current_issue 무관). 여기 도달했다면
      # current_issue가 관찰 이슈에 stale-pin된 것 — 비워서 다음 wake에 새 이슈를 픽한다.
      # 관찰 로직을 여기서 수행하지 않는다 (waiting_on_dep 케이스와 동일 패턴).
      state.current_issue = None
      orchestrator_state.write(state)
      goto Step 5

    case ("regression_detected", _):
      # Rev 19: 사용자 결정은 out-of-band로 수집된다 (Step −5가 push_pending_question →
      # Step −3가 issue.regression_decision 기록 → Step −5가 적용). inline AskUser/current_issue
      # 의존 없음. 이 케이스는 free-current_issue 폴백일 뿐.
      if state.current_issue == issue.number:
        state.current_issue = None
      orchestrator_state.write(state)
      goto Step 5
...
```json
{
  ...
  "watch_list": [
    {"pr_url":"https://github.com/.../pull/45", "issue_num":42, "pr_branch":"loopd-task-…",
     "touched_paths":["src/auth.py"], "merged_at":"…",
     "merge_commit_sha":null,            // Rev 19: MERGED 확인 시 1회 채움
     "last_checked_at":null,             // Rev 19: 폴 간격 가드(ORCHESTRATOR_OBSERVE_POLL_MIN)
     "expires_at":"…"}                   // Rev 19: 의미=안전 캡(merged_at+ORCHESTRATOR_OBSERVE_CAP_MIN), 기다리는 시간 아님
  ]
}
```
...
2. **신규 코드 최소화**: 기존 timeout 가드 (analyze=10분 / test=20분 / scout 15분 / merged_observing 안전 캡 기본 60분 — Rev 19) 가 5h+ gap을 자동 정리하므로 reconciliation 신규 분기 0건 목표.
...
| `merged_observing` | Step −5 백그라운드 sweep이 CI-완료 게이트로 advance (current_issue 무관). main CI 통과/안전 캡 → done_final, 실패·revert·cross-ref → regression_detected | §9 Step −5 (Rev 19) |
...
| **Post-merge monitoring** (Rev 19, was Rev 13 A2) | 머지 후 `merged_observing` — **non-blocking** 백그라운드 CI-완료 게이트(Step −5 sweep, current_issue 무관). main squash 커밋 CI 통과/안전 캡(기본 60분) → `done_final`, 실패·revert·cross-ref → `regression_detected` → 사용자 revert/keep/manual confirm |
